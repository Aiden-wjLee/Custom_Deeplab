...
def load_model(model_path):
    model = DeepLabv3_plus(
        nInputChannels=3, n_classes=3, os=16, pretrained=True, _print=True
    )
...
def main():
    ...
    while True:
        ...
        frame_show = cv2.resize(
            frame, (predicted_labels_image.shape[0], predicted_labels_image.shape[1])
        )

        # show predicted image
        # predicted_labels_image_3d = np.stack([predicted_labels_image[0,:,:]]*3, axis=-1)
        predicted_labels_image_3d = np.stack(
            [predicted_labels_image[:, :]] * 3, axis=-1
        )
        ...
        print(
            "max_frame_show: ",
            np.max(frame_show),
            "max_predicted_labels_image_3d: ",
            np.max(predicted_labels_image_3d),
        )
